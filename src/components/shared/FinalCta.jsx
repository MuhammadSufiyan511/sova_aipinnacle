...
export function FinalCta() {
  ...
  return (
    <section className="home-final-cta-section mx-auto max-w-[1160px] px-4 pb-16 pt-8 sm:px-5 sm:pb-20 sm:pt-10">
      <MotionDiv
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.7 }}
        className="relative overflow-hidden rounded-[26px] border border-[#2A8479] bg-[#115A54] px-4 py-8 shadow-[0_24px_60px_rgba(17,90,84,0.28)] sm:rounded-[32px] sm:px-10 sm:py-12"
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-between gap-6 sm:gap-8 md:flex-row md:text-left">
          <div className="max-w-[620px] text-center md:text-left">
            <h2 className="font-display text-[1.8rem] font-extrabold tracking-[-0.04em] text-white sm:text-[3rem] lg:text-[3.4rem]">
              {t('sections.finalTitle')}
            </h2>
            <p className="mt-3 text-[0.9rem] leading-6 text-white/84 sm:text-[0.98rem] sm:leading-[1.75]">
              {t('hero.description')}
            </p>
          </div>
