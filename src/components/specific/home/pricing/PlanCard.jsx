...
export const PlanCard = memo(function PlanCard({ plan, index, isPopular, t }) {
  return (
    <MotionDiv
      ...
      className={`relative h-full overflow-hidden rounded-[28px] border p-7 transition-all ${isPopular
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {plan.badge && (
          <span
            className={`shrink-0 rounded-full px-2.5 py-1 text-[0.7rem] font-bold tracking-wide ${isPopular ? 'bg-[#10B981] text-white' : 'bg-[#ECFDF5] text-[#10B981]'
              }`}
          >
            {plan.badge}
          </span>
      ...
      <div className="flex items-end gap-1.5">
        ...
        {plan.price.includes('$') && (
          <span className="mb-2 text-[0.82rem] font-medium text-[#F1990A]">{t('common.monthSuffix')}</span>
        )}
      </div>
      ...
      <div className="space-y-3.5">
        {plan.points.map((point) => (
          <div key={point} className="flex items-center gap-3 text-[0.875rem] text-[#1E293B]">
            <span
              className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isPopular ? 'bg-[#10B981]' : 'bg-[#ECFDF5]'
                }`}
            >
      ...
      <Link
        ...
        className={`mt-7 flex w-full items-center justify-center gap-2 rounded-full py-3 text-[0.88rem] font-bold transition hover:scale-[1.02] ${isPopular
