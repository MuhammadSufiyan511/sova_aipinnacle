...
export const Overview = memo(function Overview() {
  ...
  const LockOverlay = ({ isLocked, text = t('admin.common.unlockToView', 'Unlock to view'), title }) => {
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded-[inherit] bg-white/70 backdrop-blur-[10px] transition-all duration-300">
        <div className="flex flex-col items-center gap-2">
          <div className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-slate-900/10 shadow-sm backdrop-blur-lg">
            <Lock className="h-4 w-4 sm:h-5 sm:w-5 text-slate-700" />
          </div>
    ...
    <Motion.div variants={container} initial="hidden" animate="show" className="mx-auto flex w-[94%] flex-col gap-4 sm:w-full sm:gap-5 admin-overview-shell">
      ...
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Motion.div key={stat.name} variants={item} className="relative rounded-[22px] border border-[#DDEFE7] bg-white p-4 shadow-sm admin-stat-box flex flex-col items-center text-center sm:items-start sm:text-left group">
            <button className="absolute top-4 right-4 z-20 flex h-7 w-7 items-center justify-center rounded-full bg-slate-900/5 text-slate-400 transition-all cursor-not-allowed">
               {locks.stats ? <Lock className="h-3.5 w-3.5 opacity-50" /> : <Unlock className="h-3.5 w-3.5 text-emerald-500" />}
            </button>
            ...
            <LockOverlay isLocked={locks.stats} text={t('admin.common.locked', 'Locked')} title={stat.name} />
          </Motion.div>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1.45fr_0.95fr]">
        <Motion.div variants={item} className="relative group">
          <button className="absolute top-5 right-5 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900/5 text-slate-400 transition-all cursor-not-allowed">
             {locks.main ? <Lock className="h-4 w-4 opacity-50" /> : <Unlock className="h-4 w-4 text-emerald-500" />}
          </button>
          <div className={`transition-all duration-300 ${locks.main ? 'opacity-10 blur-sm [&>div]:pointer-events-none' : ''}`}>
             <MainChart />
          </div>
          <LockOverlay isLocked={locks.main} title={t('admin.overview.charts.saleTrend.title')} />
        </Motion.div>
        ...
        <Motion.div variants={item} className="relative rounded-[26px] border border-[#DDEFE7] bg-white p-4 shadow-sm sm:p-5 admin-chart-card group">
          <button className="absolute top-5 right-5 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900/5 text-slate-400 transition-all cursor-not-allowed">
             {locks.donut ? <Lock className="h-4 w-4 opacity-50" /> : <Unlock className="h-4 w-4 text-emerald-500" />}
          </button>
          ...
          <LockOverlay isLocked={locks.donut} title={t('admin.overview.charts.leadMix.title')} />
        </Motion.div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <Motion.div variants={item} className="relative rounded-[26px] border border-[#DDEFE7] bg-white p-4 shadow-sm sm:p-5 admin-chart-card group">
          <button className="absolute top-5 right-5 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900/5 text-slate-400 transition-all cursor-not-allowed">
             {locks.bar ? <Lock className="h-4 w-4 opacity-50" /> : <Unlock className="h-4 w-4 text-emerald-500" />}
          </button>
          <div className="flex flex-col items-center justify-between gap-3 text-center sm:flex-row sm:text-left pr-10">
            <div>
          ...
          <LockOverlay isLocked={locks.bar} title={t('admin.overview.charts.leadsByDay.title')} />
        </Motion.div>
